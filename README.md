...
| `--target <path>` | `.env` (default), `.env.local`, or `stdout`            |
...
Library opt-outs live in [`src/registry.ts`](./src/registry.ts) — the single source of truth (not duplicated here). Each entry has a stable `id` (e.g. `next`, `prisma`, `vercel`) for `--ignore` and JSON output.
